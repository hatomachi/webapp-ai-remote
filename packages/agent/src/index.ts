...
function resolveClaudeBin(): string {
  ...
  if (!isWindows && bin && (bin.endsWith('.cmd') || bin.endsWith('.bat'))) {
    console.warn(`[Agent] ⚠️ CLAUDE_BIN に '${bin}' が指定されていますが、非Windows環境 (${process.platform}) のため無視し、Linux/macOS 向けバイナリを自動探索します。`);
    bin = undefined;
  }
...
function executeClaudeTurn(params: {
  ...
  const runProcess = (resumeMode: boolean) => {
    ...
    child.on('error', (err: any) => {
      ...
      if (err.code === 'ENOENT') {
        errorDetail = `Claude Code CLI ('${CLAUDE_BIN}') が見つかりません。PATH が通っているか、または Claude Code がインストールされているか確認してください。(例: npm install -g @anthropic-ai/claude-code)`;
      }
